...
mcp = FastMCP("K8sDiagnosticTools")
...
def run_tcpdump(pod_name: str, container_name: str = None, namespace: str = "default", interface: str = "eth0", duration: int = 30, filter_expr: str = "") -> str:
    ...
    """
    logging.info(f"Starting tcpdump in pod '{pod_name}', container '{container_name}' on interface '{interface}' for {duration} seconds with filter '{filter_expr}'")
    try:
        ...
        try:
            ...
            if e.status == 404:
                logging.info(f"Error: Pod '{pod_name}' not found in namespace '{namespace}'.")
                return f"Error: Pod '{pod_name}' not found in namespace '{namespace}'."
            else:
                logging.info(f"Error accessing pod: {str(e)}")
        ...
        if container_name:
            ...
            if not selected_container:
                logging.info(f"Error: Container '{container_name}' not found in pod '{pod_name}'.")
                return f"Error: Container '{container_name}' not found in pod '{pod_name}'."
        else:
            selected_container = pod.spec.containers[0].name
        logging.info(f"Selected container: {selected_container}")  
        #logging.info(f"Checking  tcpdump command in pod '{pod_name}', container '{selected_container}' on interface '{interface}' for {duration} seconds with filter '{filter_expr}'")    
        ...
        #logging.info(f"Checking  tcpdump command resp '{resp}'")
        ...
        logging.info(f"Output file for tcpdump: {output_file}")
        filter_arg = f" '{filter_expr}'" if filter_expr else ""
        logging.info(f"Building tcpdump command with filter '{filter_arg}'")
        
        tcpdump_cmd = f"tcpdump -i {interface} -w {output_file} -G {duration} -W 1{filter_arg}"
        
        logging.info(f"Running tcpdump command in pod '{pod_name}', command = '{tcpdump_cmd}'")    
        ...
        logging.info(f"Running tcpdump command in pod '{pod_name}', command = kubectl exec -it -n {namespace} {pod_name} -c {selected_container} -- tcpdump -i {interface} -w {output_file} -G 10 -W 1")    
        try:
            result = subprocess.run(
                ["kubectl", "exec", "-it","-n",namespace,pod_name,"-c",selected_container ,"--", "tcpdump", "-i", interface, "-w", output_file, "-G", "10", "-W", "1"],
                capture_output=False,
                text=False,
                check=True,
            )
            logging.info(f"Running tcpdump Done '{result.stdout}'") 
            ...
            logging.error(f"Running tcpdump Error '{e}'")

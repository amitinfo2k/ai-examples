...
- **MCP Server**: Go-based JOLT transformation service (from jolt-transformer)
...
The JOLT transformation is handled by a separate Go-based MCP server. You'll need to build and deploy it first:

```bash
# Clone the jolt-transformer repository
cd /path/to/ai-examples/mcp-examples/jolt-transformer

# Build the Docker image
docker build -t mcp-jolt-server .

# Or pull from your container registry if already built
docker pull ghcr.io/your-org/mcp-jolt-server:latest

# Deploy to Kubernetes (from the jolt_platform_agentic directory)

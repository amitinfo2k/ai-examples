...
The JOLT Platform now supports an **Agentic Orchestrator** with Human-in-the-Loop capabilities. This allows you to review and refine JOLT specifications when automated validation fails.

### Workflow Overview

1.  **Start**: Submit a workflow request (`POST /workflow`).
2.  **Validation Failure**: If validation fails, the job status becomes `human_review_required`.
3.  **Review**: List pending reviews (`GET /reviews`).
4.  **Feedback**: Submit feedback (`POST /reviews/{job_id}/feedback`).
    *   **Approve**: Forces completion.
    *   **Reject/Refine**: Feedback is sent to the Creator Agent to regenerate the spec.
...
```json
{
  "feedback": "The field mapping for name is incorrect. It should be mapped to fullName.",
  "approve": false
}
```
...
2.  **Check Status**:
    ```bash
    curl http://localhost:8000/workflow/YOUR_JOB_ID
    ```
    If it returns `"status": "human_review_required"`, proceed to review.

3.  **Provide Feedback**:
    ```bash
    curl -X POST http://localhost:8000/reviews/YOUR_JOB_ID/feedback \
      -H "Content-Type: application/json" \
      -d '{
        "feedback": "Fix the name mapping",
        "approve": false
      }'
    ```

4.  **Monitor Refinement**:
    The Creator Agent will receive your feedback and generate a new spec. The workflow continues automatically.

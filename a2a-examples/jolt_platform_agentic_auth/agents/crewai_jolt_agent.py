...
class JoltSpecificationCreator:
    ...
    def refine_jolt_spec(self, input_json: Dict[str, Any], output_json: Dict[str, Any], current_spec: Dict[str, Any], feedback: str) -> Dict[str, Any]:
        """
        Refine a JOLT specification based on feedback.
        
        Args:
            input_json: The source JSON structure
            output_json: The desired target JSON structure
            current_spec: The current JOLT specification
            feedback: Feedback on why the current spec is incorrect or needs improvement
            
        Returns:
            A refined JOLT specification as a dictionary
        """
        task = Task(
            description=f"""
            Refine the following JOLT specification based on the provided feedback.
            ...
            Requirements:
            1. Analyze the feedback and the current spec
            2. Modify the spec to address the feedback
            3. Ensure the refined spec still handles the input/output transformation correctly
            4. Return ONLY the refined JOLT specification as valid JSON
        ...
        try:
            ...
            print(f"Error parsing refined JOLT spec: {e}")
            print(f"Raw result: {result}")
            raise ValueError(f"Failed to parse refined JOLT specification from agent output: {e}")

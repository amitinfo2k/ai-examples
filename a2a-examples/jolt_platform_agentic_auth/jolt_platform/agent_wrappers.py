...
class CreatorAgentWrapper(AgentWrapper):
    ...
    def setup_subscriptions(self):
        ...
        self.bus.subscribe("FEEDBACK_RECEIVED", self.handle_feedback)
    ...
    def handle_feedback(self, message: Message):
        print(f"\n[CreatorAgent] Received feedback: {message.type}")
        payload = message.payload
        input_json = payload.get("input_json")
        expected_output = payload.get("expected_output")
        current_spec = payload.get("jolt_spec")
        feedback = payload.get("feedback")
        job_id = payload.get("job_id")

        if not input_json or not expected_output or not current_spec or not feedback:
            print("[CreatorAgent] Error: Missing required fields for feedback refinement")
            return

        print("[CreatorAgent] Refining JOLT spec based on feedback...")
        try:
            refined_spec = self.agent.refine_jolt_spec(input_json, expected_output, current_spec, feedback)
            ...
            self.publish("SPEC_CREATED", publish_payload)
            print("[CreatorAgent] Published refined SPEC_CREATED")
        except Exception as e:
            print(f"[CreatorAgent] Error refining spec: {e}")
            error_payload = {"error": str(e), "stage": "refinement"}
            if job_id:
                error_payload["job_id"] = job_id
            self.publish("WORKFLOW_ERROR", error_payload)
...
class ValidatorAgentWrapper(AgentWrapper):
    ...
    def handle_spec_created(self, message: Message):
        ...
        try:
            ...
            # Note: We no longer publish WORKFLOW_COMPLETE here. 
            # The Orchestrator will decide whether to complete or request feedback.
        except Exception as e:
